  ...
  return (
    <div className='flex'>
      ...
      <div className='w-[78%] pt-[32px] pl-[83px] pb-[63px] border-l-[1px] border-[#D3D3D3] h-[100vh] overflow-y-auto'>
        <h1 className='font-semibold text-[32px] leading-[38.73px] text-black'>{t('dashboard.homePrice.title')}</h1>
        <button onClick={() => navigate(-1)} className='w-[186px] border-[1px] border-[#0000007a] rounded-[8px] flex items-center justify-center py-[9px] gap-[6px] mt-[50px]'>
          <ArrowIcon />
          {t('dashboard.homePrice.buttontext1')}
        </button>
        <div className='homePriceCard w-[937px] rounded-[15px] pt-[25px] pl-[94px] pr-[25px] pb-[25px] mt-[24px]'>
          <h2 className='font-semibold text-[20px] leading-[33.14px] text-black mb-[24px]'>{t('dashboard.homePrice.subtitle')}</h2>
            ...
            <ul className='mt-[23px] mb-[43px]'>
              <li className='flex flex-col gap-[10px]'>
                <div className='flex items-center justify-between text-start'>
                  <h3 className='font-semibold text-[20px] text-black leading-[33.14px]'>{t('dashboard.homePrice.realestateType')}</h3>
                  <p className='font-light text-[20px] leading-[33.14px] text-[#000000]'>{newData.estate_type}</p>
                </div>
                <div className='flex items-center justify-between text-start'>
                  <h3 className='font-semibold text-[20px] text-black leading-[33.14px]'>{t('dashboard.homePrice.cadastrNumber')}: </h3>
                  <p className='font-light text-[20px] leading-[33.14px] text-[#000000]'>{newData.cadastre_no}</p>
                </div>
                {newData.details ?
                  <>
                    <div className='flex items-center justify-between'>
                      <h3 className='font-semibold text-[20px] text-black leading-[33.14px]'>{t('dashboard.homePrice.region')}</h3>
                      <p className='w-[450px] font-light text-[20px] text-end leading-[33.14px] text-[#000000]'>{newData.details.region}</p>
                    </div>
                    <div className='flex items-center justify-between  text-start'>
                      <h3 className='font-semibold text-[20px] text-black leading-[33.14px]'>{t('dashboard.homePrice.district')}</h3>
                  ...
                  </> : <p>{t('dashboard.homePrice.noData')}</p>
                }
              </li>
            </ul>
            :
            <h1>{t('dashboard.homePrice.noData')}</h1>
          }
          {newData &&
